...
- `LlmUsageMonitor.Api/` : ASP.NET Core 10.
  ...
  - Usage en tokens : `POST /api/token-usage` reçoit les totaux horaires (poste × fournisseur × modèle) envoyés par le collecteur de l'app desktop media-tools ([Elyspio/elytools](https://github.com/Elyspio/elytools)), qui lit les journaux de session locaux. Le coût est calculé à l'envoi et stocké ; un modèle sans prix reste « Unpriced ».
